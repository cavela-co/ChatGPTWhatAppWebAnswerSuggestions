function createGptButton() {
  ...
  gptButton.innerHTML =
    '<span class="gptbtn-text">ChatGPT</span>\n' +
...
function createButton(pathVariable, title) {
  ...
  buttonElement.innerHTML =
    '<button class="svlsagor"><span><svg viewBox="0 0 24 24" height="24" width="24" preserveAspectRatio="xMidYMid meet"><path fill="green"></path></svg></span></button>';
...
function createButtonEmpty(title) {
  const buttonElement = document.createElement("button");
  buttonElement.innerHTML =
    '<button class="svlsagor"><span><svg viewBox="0 0 24 24" height="24" width="24" preserveAspectRatio="xMidYMid meet"></svg></span></button>';
  buttonElement.setAttribute("title", title);
...
function createAndAddOptionsButton(newButtonContainer) {
  ...
  optionsButton.addEventListener("click", () => {
    try {
      ...
      // ignore, sometimes happens when reloading the extension, but don't want to see it pop up in the console
    }
  });
...
function creatCopyButton(newFooter, newButtonContainer) {
  const { svg: svgElement, buttonElement: copyButton } = createButtonEmpty(
    "Copy chat suggestion"
  );
  // console.log('copyButton2: ', copyButton);
  svgElement.innerHTML =
    '<path d="M3,13c0-2.45,1.76-4.47,4.08-4.91L5.59,9.59L7,11l4-4.01L7,3L5.59,4.41l1.58,1.58l0,0.06C3.7,6.46,1,9.42,1,13 c0,3.87,3.13,7,7,7h3v-2H8C5.24,18,3,15.76,3,13z"/><path d="M13,13v7h9v-7H13z M20,18h-5v-3h5V18z"/><rect height="7" width="9" x="13" y="4"/>\n';
...
function createGptFooter(footer, mainNode) {
  ...
  setTimeout(() => {
    const div = mainNode.querySelectorAll(
